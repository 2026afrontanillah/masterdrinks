...
function construirPdfCierre(datos) {
  ...
  if (resumen.cortesias > 0) {
    doc.parrafo(`Se emitieron ${resumen.cortesias} comandas de cortesía (valorizadas en ${num2(resumen.importe_cortesia)} Bs.) ` +
                'que no se suman a lo recaudado en caja ni se exigen al cajero.',
      { color: COLOR.tinta });
    doc.espacio(4);
  }
  ...
  if (datos.porMesero.length > 0) {
    ...
    doc.tabla({
      ...
      filas: datos.porMesero.map(m => [
        m.mesero + (m.es_cortesia ? ' [Cortesías]' : ''),
        String(m.comandas),
        num2(m.importe)
      ])
    });
  ...
  if (reimpresiones.length > 0) {
    ...
    doc.parrafo('Cada línea es una vez que se volvió a sacar el papel de una venta ya ' +
                'cobrada, de la más reciente a la más antigua. Deja al cajero y al mesero que ' +
                'estaban en turno: una misma comanda repetida varias veces merece una ' +
                'explicación.');
    doc.tabla({
      columnas: [
        { titulo: 'Comanda', ancho: 12, align: 'right' },
        { titulo: 'Reimpr.', ancho: 10, align: 'right' },
        { titulo: 'Cajero', ancho: 26 },
        { titulo: 'Mesero', ancho: 22 },
        { titulo: 'Hora', ancho: 12 }
      ],
      filas: reimpresiones.map(r => [
        String(r.id_comanda),
        String(r.numero_reimpresion) + 'a',
        r.cajero,
        r.mesero,
