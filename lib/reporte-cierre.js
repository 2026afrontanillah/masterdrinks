 ...
 * Lleva la barra dentro: si llegan tres cierres al mismo chat y se llaman
 * igual, el segundo pisa al primero y nadie sabe cuál abrió.
 */
...
function construirPdfCierre(datos) {
  ...
  // ---- Cabecera --------------------------------------------------------
  const barra = datos.instancia && datos.instancia.nombre ? datos.instancia.nombre : null;
  doc.cabecera('MASTERDRINKS',
    barra ? 'Cierre de caja · ' + barra : 'Reporte de cierre de caja',
    textoRango(datos.rango));
  if (barra) {
    doc.parrafo(`Este cierre cubre únicamente la barra ${barra}` +
      (datos.instancia.prefijo ? ` (comandas ${datos.instancia.prefijo}-...)` : '') +
      '. Cada barra lleva su propio servidor, su propio stock y su propia numeración.',
      { color: COLOR.tinta });
  }
  if (datos.evento) {
    doc.parrafo(`${datos.evento.nombre_evento} · ${datos.evento.lugar || 'Sin lugar'} · ` +
                `${datos.evento.fecha_evento || ''}`);
  }
  doc.espacio(4);
  ...
  if (datos.anuladas.length > 0) {
    ...
    doc.tabla({
      ...
      filas: datos.anuladas.map(a => [
        (datos.instancia && datos.instancia.prefijo ? datos.instancia.prefijo + '-' : '#') + a.id_comanda,
