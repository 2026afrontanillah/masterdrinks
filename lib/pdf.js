...
const COLOR = {
  ...
  blanco: [1, 1, 1],
  grisAnulado: [0.89, 0.90, 0.92],
  textoAnulado: [0.44, 0.46, 0.50]
};
...
class Documento {
  ...
  /** Parte un texto en líneas que caben en el ancho dado, respetando saltos de línea y palabras. */
  ...
  /**
   * Tabla con cabecera, cebra, fondos personalizados y salto de página automático.
   * @param {object} tabla
   *   columnas        [{ titulo, ancho (proporción), align, fuente, multilinea, tamano }]
   *   filas           [[celda, ...] | { celdas: [...], fondo: [r,g,b], colorTexto: [r,g,b] }]
   *   totales         fila final destacada (opcional)
   *   separadorFilas  dibuja línea divisoria entre filas si es true (opcional)
   */
  tabla({ columnas, filas, totales, separadorFilas }) {
    const altoFila = 16;
    ...
    filas.forEach((fila, indice) => {
      ...
      const colorFila = (fila && fila.colorTexto) ? fila.colorTexto : null;
      ...
      const contenido = celdas.map((celda, i) => {
        ...
        const fuente = (celda && typeof celda === 'object' && celda.fuente) || col.fuente || (align === 'right' ? 'mono' : 'normal');
        const tamano = (celda && typeof celda === 'object' && celda.tamano) || col.tamano || 8.5;
        const esMultilinea = (celda && typeof celda === 'object' && celda.multilinea !== undefined) ? celda.multilinea : col.multilinea;

        let lineas;
        if (celda && typeof celda === 'object' && Array.isArray(celda.lineas)) {
          lineas = celda.lineas;
        } else {
          const valor = String((celda && typeof celda === 'object' ? celda.texto : celda) ?? '');
          lineas = esMultilinea
            ? this.partir(valor, anchos[i] - 12, tamano, fuente)
            : [recortar(valor, anchos[i] - 12, tamano, fuente)];
        }

        return {
          lineas,
          color: (celda && typeof celda === 'object' && celda.color) || colorFila || COLOR.tinta,
          tamano,
          align,
          fuente
        };
      ...
      if (separadorFilas || (fila && fila.separador)) {
        this.linea(this.margen, y, this.ancho - this.margen, y, COLOR.linea, 0.4);
      }
    ...
    if (totales) {
      ...
      totales.forEach((celda, i) => {
        ...
        const colorValor = (celda && typeof celda === 'object' && celda.color) || COLOR.tinta;
        const fuenteValor = (celda && typeof celda === 'object' && celda.fuente) || (align === 'right' ? 'monoNegrita' : 'negrita');
        this.texto(textoValor, px, y + 4.5,
          { tamano: 9, fuente: fuenteValor, color: colorValor, align });
        x += anchos[i];
      });
      this.y = y;
    }
